...
def main():
    ...
    df = df[df[col_query_locale] == args.locale]  ##筛选出同一语言的 es/us/jp
    ...
    df = df[[col_query_id, col_query, col_product_title, col_gain]] ##取查询id，查询内容，结果名称，结果的gain
    df_train = df[df[col_query_id].isin(list_query_id_train)]
    df_dev = df[df[col_query_id].isin(list_query_id_dev)]
    
    """ 2. Prepare data loaders """
    train_samples = []
    for (_, row) in df_train.iterrows(): ## iterrows 可以返回所有的行索引，以及该行的所有内容
